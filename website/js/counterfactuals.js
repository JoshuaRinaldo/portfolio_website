...
// Get DOM elements
const form = document.getElementById('counterfactual-form');
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const mlmEndpoint = ENDPOINT_NAMES.SENTIMENT_UNMASKING_MODEL;

        const requestBody = {
            input_text: text,
            classification_model: classificationEndpoint,
            masked_language_model: mlmEndpoint,
            desired_class: modelConfig.desired_label,
            undesired_class: modelConfig.undesired_label,
        };
        ...
        showError('An error occurred while generating counterfactuals. Please try again later.');
    } finally {
...
function displayResults(data, modelType) {
    ...
    // Display explanation
    if (data.endpoint_response && data.endpoint_response.explanation) {
        const explanationHtml = createHexCss(
            data.endpoint_response.explanation,
            desiredLabel,
            undesiredLabel
        );

        html += `
            <h3>Your Input's Explanation:</h3>
            <div class="explanation">
                ${explanationHtml}
            </div>
            <p style="text-align: center; font-size: 0.9rem; color: #666;">
                (green indicates a contribution towards the desired label (${desiredLabel}),
                red indicates a contribution towards the undesired label (${undesiredLabel}))
            </p>
        `;
    }

    // Display counterfactuals table
    html += '<h3>Counterfactual Generation Response:</h3>';
    ...
    if (data.result && data.result.length > 0) {
        ...
        for (const counterfactual of data.result) {
            const [text, label, score] = counterfactual;
            html += `
                <tr>
        ...
        html += '<p>No counterfactuals were generated.</p>';
    }
